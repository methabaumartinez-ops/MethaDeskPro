...
export default function PositionErfassenPage() {
    ...
    // Fetch Teilsystem details to display name/number
    // @ts-ignore
    const teilsystem = mockStore.getTeilsysteme().find((t: any) => t.id === teilsystemId);

    const onSubmit = async (data: PositionValues) => {
        await new Promise((resolve) => setTimeout(resolve, 800));
        const all = mockStore.getPositionen();
        const newItem = {
            id: `pos-${Date.now()}`,
            ...data,
        ...
        console.log("Saving new position:", newItem);
        alert("Position gespeichert (Mock)");
