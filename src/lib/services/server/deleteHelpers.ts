...
export async function deleteTeilsystemWithCascade(id: string) {
    ...
    const positions = await DatabaseService.list<any>('positionen', { filter: { must: [{ key: 'teilsystemId', match: { value: id } }] } });
    ...
    const allDocs = await DatabaseService.list<any>('dokumente', { filter: { must: [{ key: 'entityId', match: { value: id } }] } });
    for (const pos of positions) {
        const posDocs = await DatabaseService.list<any>('dokumente', { filter: { must: [{ key: 'entityId', match: { value: pos.id } }] } });
...
export async function deletePositionWithCascade(id: string) {
    ...
    const docs = await DatabaseService.list<any>('dokumente', { filter: { must: [{ key: 'entityId', match: { value: id } }] } });
